    ...
    public function cobrar(Request $request, CuentaPorCobrar $cuenta): RedirectResponse
    {
        ...
        try {
            DB::transaction(function () use ($cuenta, $datos, $apertura, $usuario) {
                // se relee con bloqueo: un doble clic o dos cajeros no pueden cobrar de mas
                $bloqueada = CuentaPorCobrar::lockForUpdate()->findOrFail($cuenta->id);
                ...
                if ($bloqueada->estado === 'pagado' || $monto > $saldo + 0.001) {
                    throw new ErrorDeNegocio('El saldo cambió mientras registrabas el cobro (ahora es S/ '.number_format($saldo, 2).'). Revisa y vuelve a intentarlo.');
                }
